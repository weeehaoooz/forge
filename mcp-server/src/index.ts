...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case 'talos_get_component_doc': {
      ...
      const component = TALOS_COMPONENTS.find(
      ...
      if (!component) {
        ...
        throw new McpError(
          ...
          `Component "${target}" not found. Available components: ${available}`
        );
      }
    ...
    case 'talos_get_example': {
      const target = String(args?.componentId || '').toLowerCase().trim();
      const examples = TALOS_EXAMPLES.filter(
        (ex) => ex.componentId.toLowerCase() === target || ex.id.toLowerCase() === target
      );
      ...
      if (examples.length === 0) {
        ...
        throw new McpError(
          ErrorCode.InvalidParams,
          `No examples found for "${target}". Available examples for: ${available}`
        );
      }
    ...
    case 'talos_get_styles_and_tokens': {
      ...
      let result: unknown = TALOS_DESIGN_TOKENS;
      if (section === 'colors') result = TALOS_DESIGN_TOKENS.colorSystem;
      else if (section === 'typography') result = TALOS_DESIGN_TOKENS.typography;
      else if (section === 'forms') result = TALOS_DESIGN_TOKENS.formControls;
      else if (section === 'buttons') result = TALOS_DESIGN_TOKENS.buttons;
      else if (section === 'dark-mode') result = TALOS_DESIGN_TOKENS.darkMode;
      else if (section === 'setup') result = { summary: TALOS_DESIGN_TOKENS.summary, importGuide: TALOS_DESIGN_TOKENS.importGuide };
    ...
    case 'talos_generate_template': {
      const componentName = (args?.componentName as string) || 'CustomWidget';
      const usedIds = (args?.usedComponents as string[]) || [];
