...
export type TalosSnackbarVariant = 'default' | 'info' | 'success' | 'warning' | 'error';

export type TalosSnackbarPosition =
  | 'top-left'
  | 'top-center'
  | 'top-right'
  | 'bottom-left'
...
export interface TalosSnackbarConfig {
...
export interface TalosSnackbarRef {
...
export interface TalosSnackbarInstance extends Required<Omit<TalosSnackbarConfig, 'title' | 'actionLabel' | 'onAction' | 'onDismiss' | 'customClass' | 'icon' | 'data'>> {
  id: string;
  title?: string;
  actionLabel?: string;
  onAction?: () => void;
