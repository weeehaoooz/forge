  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => ForgeRadioGroupComponent),
      multi: true
    }
  ],
  host: {
    'class': 'forge-radio-group',
    'role': 'radiogroup',
...
export class ForgeRadioGroupComponent implements ControlValueAccessor {
  private readonly elementRef = inject(ElementRef<HTMLElement>);

  // Signal Inputs
  readonly name = input<string>(`forge-radio-group-${++groupUniqueIdCounter}`);
  ...
  readonly radios = contentChildren(ForgeRadioComponent, { descendants: true });
