...
import { CheckboxDirection, ForgeCheckboxGroupDirective } from './checkbox-group.directive';
import { CheckboxSize, CheckboxVariant } from './checkbox.directive';

@Component({
  selector: 'forge-checkbox-group',
  template: '<ng-content></ng-content>',
  hostDirectives: [
    {
      directive: ForgeCheckboxGroupDirective,
      inputs: ['name', 'size', 'variant', 'direction', 'disabled', 'invalid'],
      outputs: ['valueChange']
    }
  ]
})
export class ForgeCheckboxGroupComponent {
  readonly groupDir = inject(ForgeCheckboxGroupDirective, { self: true });
