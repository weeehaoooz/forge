...
@Directive({
  selector: '[forgeCheckboxGroup], [forge-checkbox-group]',
  exportAs: 'forgeCheckboxGroup',
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => ForgeCheckboxGroupDirective),
      multi: true
    }
  ],
  host: {
    'class': 'forge-checkbox-group',
    'role': 'group',
...
export class ForgeCheckboxGroupDirective implements ControlValueAccessor {
  readonly elementRef = inject(ElementRef<HTMLElement>);

  /** Name attribute for the checkbox group */
  readonly name = input<string>(`forge-checkbox-group-${++groupCounter}`);
