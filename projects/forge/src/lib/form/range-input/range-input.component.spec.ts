...
import { ForgeRangeInputModule } from './range-input.module';
import { RangeInputValue } from './range-input.types';

@Component({
  imports: [ForgeRangeInputModule, ReactiveFormsModule],
  template: `
    <forge-range-input
      [formControl]="control"
      [min]="min()"
...
describe('ForgeRangeInputComponent Suite', () => {
  ...
  it('should initialize single slider with default value from FormControl', () => {
    const thumb = fixture.nativeElement.querySelector('.forge-range-thumb') as HTMLElement;
  ...
  it('should render interval ticks and labels when configured', () => {
    ...
    const ticks = fixture.nativeElement.querySelectorAll('.forge-range-tick');
    expect(ticks.length).toBe(5);

    const labels = fixture.nativeElement.querySelectorAll('.forge-range-label-item');
  ...
  it('should support dual-handle range mode', () => {
    ...
    const thumbs = fixture.nativeElement.querySelectorAll('.forge-range-thumb');
  ...
  it('should respect disabled state from FormControl', () => {
    ...
    const wrapper = fixture.nativeElement.querySelector('.forge-range-wrapper');
