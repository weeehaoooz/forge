...
import { ForgeButtonDirective, ButtonVariant, ButtonSize } from './button.directive';
...
describe('ForgeButtonDirective', () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      imports: [ForgeButtonDirective, TestHostComponent]
    }).compileComponents();
  ...
  it('should apply base class and default primary variant & md size', () => {
    expect(buttonEl.classList.contains('forge-btn')).toBe(true);
    expect(buttonEl.classList.contains('forge-btn-primary')).toBe(true);
    expect(buttonEl.classList.contains('forge-btn-md')).toBe(true);
  });

  it('should dynamically update variant classes', () => {
    hostComponent.variant.set('danger');
    fixture.detectChanges();
    expect(buttonEl.classList.contains('forge-btn-danger')).toBe(true);
    expect(buttonEl.classList.contains('forge-btn-primary')).toBe(false);

    hostComponent.variant.set('outline');
    fixture.detectChanges();
    expect(buttonEl.classList.contains('forge-btn-outline')).toBe(true);

    hostComponent.variant.set('secondary');
    fixture.detectChanges();
    expect(buttonEl.classList.contains('forge-btn-secondary')).toBe(true);

    hostComponent.variant.set('ghost');
    fixture.detectChanges();
    expect(buttonEl.classList.contains('forge-btn-ghost')).toBe(true);

    hostComponent.variant.set('subtle');
    fixture.detectChanges();
    expect(buttonEl.classList.contains('forge-btn-subtle')).toBe(true);

    hostComponent.variant.set('success');
    fixture.detectChanges();
    expect(buttonEl.classList.contains('forge-btn-success')).toBe(true);

    hostComponent.variant.set('link');
    fixture.detectChanges();
    expect(buttonEl.classList.contains('forge-btn-link')).toBe(true);
  });

  it('should dynamically update size classes', () => {
    hostComponent.size.set('sm');
    fixture.detectChanges();
    expect(buttonEl.classList.contains('forge-btn-sm')).toBe(true);

    hostComponent.size.set('lg');
    fixture.detectChanges();
    expect(buttonEl.classList.contains('forge-btn-lg')).toBe(true);
  });
  ...
  it('should toggle modifier classes (iconOnly, fullWidth, pill)', () => {
    ...
    expect(buttonEl.classList.contains('forge-btn-icon')).toBe(true);
    expect(buttonEl.classList.contains('forge-btn-full')).toBe(true);
    expect(buttonEl.classList.contains('forge-btn-pill')).toBe(true);
  });
