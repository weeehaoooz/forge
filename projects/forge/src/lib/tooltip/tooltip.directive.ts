...
@Component({
  selector: 'forge-tooltip-content',
  template: `<div class="forge-tooltip-inner">{{ text() }}</div>`,
  host: {
    'class': 'forge-tooltip-panel',
    'role': 'tooltip'
  }
})
export class ForgeTooltipComponent {
  readonly text = input<string>('');
}

@Directive({
  selector: '[forgeTooltip], [tooltip]',
  host: {
    '(mouseenter)': 'show()',
    '(mouseleave)': 'hide()',
    '(focus)': 'show()',
...
export class ForgeTooltipDirective {
  private readonly elementRef = inject<ElementRef<HTMLElement>>(ElementRef);
  private readonly overlay = inject(Overlay);
  private readonly destroyRef = inject(DestroyRef);
  private readonly injector = inject(Injector);

  readonly tooltip = input<string>('', { alias: 'forgeTooltip' });
  ...
  show(): void {
    ...
    const portal = new ComponentPortal(ForgeTooltipComponent, null, this.injector);
